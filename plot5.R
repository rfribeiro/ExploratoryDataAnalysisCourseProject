...
plot5 <- function() {
  ...
  total_baltimore <- NEI %>% subset(fips == "24510" & type == "ON-ROAD") %>% group_by(year) %>% summarise(Total = sum(Emissions))
  
  png(file = "plot5.png", width = 640, height = 480)
  
  p <- ggplot(total_baltimore, aes(year, Total)) + geom_point() + geom_line() + guides(colour=FALSE) + ggtitle('Total Emissions of Motor Vehicle Sources in Baltimore City, Maryland') + geom_text(aes(label=round(Total), size=1, hjust=0.5, vjust=2))
