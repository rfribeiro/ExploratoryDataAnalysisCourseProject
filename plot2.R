...
plot2 <- function() {
  ...
  total_baltimore <- NEI %>% subset(fips == "24510") %>% group_by(year) %>% summarise(Total = sum(Emissions))
  
  png(file = "plot2.png", width = 480, height = 480)
  
  plot(total_baltimore, pch=20, type="o", main = "Total Emissions by Year at Baltimore")
